...
for wordnumber, timestamp in enumerate(timestamps, start=1):
    ...
    command = f"""ffmpeg -i {input_video}{wordnumber}.mp4 -i word_images/word_{wordnumber}.png -filter_complex "[0:v][1:v] overlay=(W-w)/2:H-h-25:enable='between(t, {tsstart}, {tsstop})'" -pix_fmt yuv420p -c:a copy {input_video}{wordnumber+1}.mp4"""
